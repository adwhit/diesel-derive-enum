...
we wish to use with Diesel must implement various traits. Tedious to do by hand,
easy to do with a macro - enter `diesel-derive-enum`.
...
Note that for now, this **only** works with Postgres - for other databases see
the next section.
...
Now we can use `MyEnum` with `diesel`.
